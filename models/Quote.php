...
<?php
    ...
    class Quote{
        ...
        public function fetchAllQuotes(){
            $query = "SELECT quotes.id, quotes.body, users.firstName, users.lastName
            categories.name As categoryName FROM quotes 
        ...
        public function fetchOneQuote($parameter){
            $query = "SELECT quotes.id, quotes.body, users.firstName, users.lastName
            categories.name As categoryName FROM quotes 
            LEFT JOIN categories ON quotes.category_id = categorues.id 
            LEFT JOIN users ON quotes.user_id = users.id 
            WHERE quotes.id=?";
        ...
        public function fetchRandomQuote($limit){
            $query = "SELECT quotes.id, quotes.body, users.firstName, users.lastName
            categories.name As categoryName FROM quotes 
            LEFT JOIN categories ON quotes.category_id = categorues.id 
            LEFT JOIN users ON quotes.user_id = users.id 
        ...
        public function updateQuote($parameters){
            $query = "UPDATE quotes SET body=?, category_id=?, date=? WHERE id ?";
            if(isset($parameters['body']) && isset($parameters['id']) && isset($parameters['category_id']) && isset($parameters['date'])){
                $id = $parameters['id'];
                $body = $parameters['body'];
                $category_id = $parameters['category_id'];
                $date = $parameters['date'];
                $results = $this->db->updateOne($query, $body, $category_id, $date, $id);
